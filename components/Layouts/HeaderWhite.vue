<template lang="pug">
	nav.header#Nav_header(v-bind:style="[url === 'detail' && screen < 700 ? {'background-color': 'rgba(0,0,0,0.83)'} : {'background-color': 'transparent'}]")
		div.Nav_header
			.Nav_header_brand_link
				img(:src="logoWhite" @click="toHome")
				div(v-bind:style="[url === 'detail' && screen < 700 ? {'display': 'none'} : {'display': 'block'}]") Renti
			.Nav_header_mobile.open
				font-awesome-icon.icon(:icon="['fas', 'bars']" @click="classToggle")
		ul.Nav_container
			//- li.Nav_item Publica tu alojamiento
			li.Nav_item
...
<script>
import logoWhite from "@/assets/images/logos/logo-home.png";
